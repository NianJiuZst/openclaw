...
describe("Windows MSYS2 Git native filesystem proof", () => {
  ...
  it("lists branches and gives primary and linked checkouts the same repository identity", async () => {
  ...
  it("preserves the native path and foreign lock reason from real porcelain output", async () => {
    const entries = await listGitWorktrees(repo);
    const linkedEntry = entries.find((entry) => path.resolve(entry.path).toLowerCase() === path.resolve(linked).toLowerCase());
